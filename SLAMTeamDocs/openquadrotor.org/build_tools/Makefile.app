...
.SECONDARY:		$(OBJS) $(QOBJS) $(COBJS)
.PHONY:			all clean copy doc debug

$(QOBJS): %.o: %.cpp moc_%.cpp
	@$(MESSAGE) "Compiling (QT) $@"
	@$(PRETTY) "$(CXX) $< -o $@ -L$(LIBDIR) $(LDFLAGS) $(LIBS)"
...
$(BINDIR)/%: %.o $(OBJS) $(QOBJS) $(COBJS)
	@$(MESSAGE) "Linking application `basename $@`"
	@$(PRETTY) "$(CXX) $(LDFLAGS) $(CPPFLAGS) $(CXXFLAGS) $(OBJS) $(QOBJS) $(COBJS) $< -L$(LIBDIR) $(LIBS) -o $@"
...
OBJDEPS=$(foreach module,$(basename $(OBJS) $(QOBJS)),$(module).d) $(foreach a, $(APPS),$(a).d)
