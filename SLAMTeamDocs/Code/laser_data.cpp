...
const double RADIUS_TO_UP_MIRROR_REFLECT_POINT = .109; // In meters
...
public:
	SplitLaserReadingCalculateAltitudeAndHeight() {
		...
		// Initialize publishers for valid altitude LRF readings, height of area estimation
		altitudePub = nodeHandle.advertise<sensor_msgs::LaserScan>(ALTITUDE_OUT, QUEUE_SIZE);
		heightPub = nodeHandle.advertise<std_msgs::Float32>(ESTIMATED_HEIGHT_OUT, QUEUE_SIZE);
		
		ros::Rate loop_rate(REFRESH_RATE);
		
		downRunningWeightedAvg = 0;
		upRunningWeightedAvg = 0;
		estimatedAreaHeight = 0;
	...
	void imuReadingCallback(const sensor_msgs::LaserScan reading) {
		//imu_roll = reading.roll;
		//imu_pitch = reading.pitch;
	}
...
private:
		...
		if(downVariance > MAX_VARIANCE_THRESHOLD) {
			if(upVariance > MAX_VARIANCE_THRESHOLD) {
				...
				for(int i = 0; i < up.ranges.size(); ++i)
					up.ranges[i] = estimatedAreaHeight - up.ranges[i];
				output = up;
			}
		} else {
			output = down;
		}
		if(validData && estimatedAreaHeight != 0) {
			output.header.frame_id = "altitude";
	...
	void calculateUpDownVariancesAdjustUpDownIMUAndEstimateHeight() {
		...
		if(downVariance < MAX_VARIANCE_THRESHOLD && upVariance < MAX_VARIANCE_THRESHOLD)
			estimatedAreaHeight = calculateMeanOfRanges(up) + calculateMeanOfRanges(down);
		std_msgs::Float32 val;
		val.data = estimatedAreaHeight;
		heightPub.publish(val);
	}
