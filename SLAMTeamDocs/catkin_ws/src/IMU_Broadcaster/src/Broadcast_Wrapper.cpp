#include <../include/IMU_Broadcaster/mavlink/include/mavlink/v1.0/common/mavlink.h>
#include <vector>

#include "../include/IMU_Broadcaster/Serial_Wrapper.h"
...
int main (int argc, char** argv) {
    ...
    ros::init(argc, argv, "msg_broadcaster");
    ros::NodeHandle node;
    ros::Publisher msg_pub = node.advertise<std_msgs::String>("msg_broadcast", 1000);
	...
	char broadcastString[500];
		...
		// broadcast the mavlink message data to ROS
		std_msgs::String broadcast;
		sprintf(broadcastString, 
			"Received packet: SYS: %d, COMP: %d, LEN: %d, MSGID: %d\n",
                        rcvMsg.sysid, rcvMsg.compid, rcvMsg.len, rcvMsg.msgid);
		broadcast.data = broadcastString;
		std::cout << "cout: " << broadcast.data.c_str() << std::endl;
		ROS_INFO("Broadcast Sent: %s", broadcast.data.c_str());
		msg_pub.publish(broadcast);
		ros::spinOnce();
		loop_rate.sleep();
