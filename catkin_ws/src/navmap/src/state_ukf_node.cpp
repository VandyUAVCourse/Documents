#include "navmap/state_ukf_node.h"
...
StateUKF::StateUKF() {
    ...
    mPub = mN.advertise<navmap::state_ukf_msg>("state_ukf", 1000);
    
    // Create the subscribers to the IMU data nd the pose estimate UKF data
    mImuSub = mN.subscribe("imu_raw", 1, &StateUKF::imu_data_receiver, this);
 ...
 */
void StateUKF::imu_data_receiver(const imu_broadcast::raw_imu& msg) {
    mControl << msg.xacc / 100.0, msg.yacc / 100.0, msg.zacc / 100.0,
 ...
 */
void StateUKF::pose_est_receiver(const navmap::pose_ukf_msgConstPtr& msg) {
